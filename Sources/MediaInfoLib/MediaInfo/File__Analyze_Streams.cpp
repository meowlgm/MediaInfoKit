...
void File__Analyze::Duration_Duration123(stream_t StreamKind, size_t StreamPos, size_t Parameter)
{
    ...
    for (size_t Pos=0; Pos<List.size(); Pos++)
    {
        ...
        // Ensure that the default display string ("/String") uses the full format with seconds
        // so that Inform() and downstream localization include seconds even when HH > 0.
        Fill(StreamKind, StreamPos, Parameter+1, DurationString1); // /String (now long format)
        Fill(StreamKind, StreamPos, Parameter+2, DurationString1); // /String1 (full format)
        Fill(StreamKind, StreamPos, Parameter+3, DurationString2); // /String2 (short format)
